...
- [Landing page design rationale](#landing-page-design-rationale)
- [API usage](#api-usage)
...
- **Color palette**: Reuses the app's own dark theme and single blue accent instead of a separate marketing palette, so the page feels continuous with `/login` and `/chat`.
- **Typography**: Manrope everywhere for the same continuity, with a large tight-tracked headline and small muted supporting copy so the live conversation stays the focal point.
- **Interactions**: Motion demonstrates real product behavior (messages entering, typing indicators, delivery ticks, a pulsing CTA) and is gated by `prefers-reduced-motion` and viewport visibility.
...
- **Landing page**: scripted demo timelines styled to match the production chat UI (not the stateful components themselves, which need real conversation data)
